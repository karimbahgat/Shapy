...
def tip_decorator_round(a,b,c,d,is_cw):
        # not done...
        return []
...
##				return (b - a) / 2.0 + a
		...
		import pydraw
                crs = pydraw.CoordinateSystem([-58.071399886445136, 1.8362450855291712, -53.986117882640414, 6.0018090894086527])
		...
		for poly in raw:
			...
			wn = winding_number(p, raw) #NOTE! Doing wn on [poly] instead helps in many cases but not all
			...
			if wn > 0: #modified, orig wn == 1 on right side
				output.append(Polygon.from_pointlist(poly))
        			img.drawpolygon([(ver.x,ver.y) for ver in poly], fillcolor=(0,random.randrange(0,255),0), outlinecolor=(0,0,0))
			else: img.drawpolygon([(ver.x,ver.y) for ver in poly], fillcolor=(random.randrange(0,255),0,0), outlinecolor=(0,0,0))

		# run union to combine all final offset poly parts
		# ...
		img.drawcircle(-55.091, 5.805, fillsize=20, outlinecolor=(0,0,222), fillcolor=None)
		img.view()
